--- Lua 5.5 bytecode file reader.
-- Parses compiled Lua 5.5 binary chunks into structured prototype tables
...
--
-- Key differences from the Lua 5.4 reader:
--   - Varint encoding: high bit 1 = more bytes (opposite of 5.4)
--   - loadFunction: source string read after protos (not before)
--   - loadCode: alignment to instruction size after reading count
--   - loadString: string deduplication (size==0 reads index)
-- @module llx.bytecode.lua55.bcode

local environment = require 'llx.environment'
local bytestream_module = require 'llx.bytecode.lua55.bytestream'
local instructions_module = require 'llx.bytecode.lua55.instructions'
local typetags_module = require 'llx.bytecode.lua55.typetags'
...
-- Reads the instruction count, aligns to instruction size, and then
-- reads each 32-bit instruction word.
...
local function load_code(proto, meta, stream)
  ...
  stream:read_align(meta.instruction_size)
  proto.code = {}
  for i=1, sizecode do
...
-- In Lua 5.5, strings support deduplication: when size==0, a varint
-- index is read to reference a previously encountered string. New
-- strings are added to a string table for future deduplication.
-- @param proto the prototype table
-- @param meta metadata table (contains the string dedup table)
-- @param stream the ByteStream to read from
-- @return the decoded string, or nil if absent
local function load_string(proto, meta, stream)
  local size = stream:read_size()
  if size == 0 then
    local idx = stream:read_varint()
    if idx == 0 then
      return nil
    end
    return meta.strings[idx]
  end
  local s = stream:read_string_vector(size - 2)
  stream:read_int8()  -- skip null terminator
...
-- of integers and floats. The header format is the same as Lua 5.4.
-- @param proto the prototype table to populate
...
-- In Lua 5.5, the source string is read after nested prototypes
-- (unlike 5.4 where it was the first field).
-- @param proto the prototype table to populate
-- @param meta metadata table
-- @param stream the ByteStream to read from
function load_function(proto, meta, stream)
  ...
  proto.flag = stream:read_int8() & ~PF_FIXED
...
-- @param bytes a binary string containing a complete Lua 5.5 compiled chunk
