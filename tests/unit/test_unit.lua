...
describe('unit test framework', function()
  ...
  it('should support Listwise matcher', function()
    ...
    expect({1, 2, 3}).to.match(
      Listwise(function(v) return Not(Equals(v)) end,
               {2, 4, 6}))
    expect({1, 2, 3}).to_not.match(Listwise(Equals, {1, 2, 4}))
    expect({1, 2, 3}).to_not.match(
      Listwise(function(v) return Not(Equals(v)) end,
               {1, 2, 3}))
    expect({1, 2, 3}).to_not.match(
      Listwise(function(v) return Not(Equals(v)) end,
  ...
  it('should support Tablewise matcher', function()
    ...
    expect({a=100, b="hello"}).to.match(
      Tablewise(function(v) return Not(Equals(v)) end,
                {a=1000, b="goodbye"}))
    expect({a=100, b="hello"}).to_not.match(
      Tablewise(Equals, {a=100, b="hello", c="world"}))
    expect({a=100, b="hello", c="world"}).to_not.match(
      Tablewise(Equals, {a=100, b="hello"}))
  end)
  ...
  it('should fail when given bad values for be_not_equal_to', function()
    ...
    local success5 = pcall(function()
      expect("hello, world").to_not.be_equal_to(
        "hello, world")
    end)
    expect(success5).to.be_false()
