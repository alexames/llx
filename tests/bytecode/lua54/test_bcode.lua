...
-- apply when that VM is Lua 5.4 (tests/init.lua gates them the same
-- way for the aggregate runner).
if _VERSION ~= 'Lua 5.4' then
  print('Skipping lua54 bytecode reader tests under ' .. _VERSION)
  return
end

describe('bytecode reader', function()
  ...
  it('works with vararg functions', function()
    local bytes = string.dump(function(...) end)
    local proto, meta = bcode.read_bytes(bytes)
    expect(proto.flag).to.be_equal_to(1)
  end)
end)
